...
which is nonsensical. Also, this causes business logic to potentially be scattered, i.e. two callers can query the same thing 
...
For some business rules we can establish clear boundaries between entities. For example, for a social feed functionality,
...
our application truly revolves around the rich domain logic, its aggregate roots, as opposed to a Generic Repository 
which, really, is a wrapper around database operations. We could have domain methods such as GetLikesAssociatedWithPost(),
which would work regardless of whether the Infrastructure layer implements EF Core, stored procedures, a file, an external API, etc.
