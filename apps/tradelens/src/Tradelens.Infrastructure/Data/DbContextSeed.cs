...
public static class DbContextSeed
{
    public static async Task SeedPostsAsync(TradelensDbContext context)
    {
        ...
        if (!context.Posts.Any())
        {
            ...
            // in development
            // var postsData = await File.ReadAllTextAsync("../Tradelens.Infrastructure/Data/SeedData/posts.json");
            
            // for docker-compose (?)
            // var path = Path.Combine("Tradelens.Infrastructure", "Data", "SeedData", "posts.json");
    ...
    public static async Task SeedStocksAsync(TradelensDbContext context)
    {
        ...
        if (!context.Stocks.Any())
        {
            ...
            // var stocksData = await File.ReadAllTextAsync("../Tradelens.Infrastructure/Data/SeedData/tickers.json");
            
            // var path = Path.Combine("Tradelens.Infrastructure", "Data", "SeedData", "tickers.json");
    ...
    public static async Task SeedCompanyMetricsAsync(TradelensDbContext context)
    {
        ...
        {
            var path = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            // var basePath = "../Tradelens.Infrastructure/Data/SeedData/CompanyFundamentalsSecParse";
